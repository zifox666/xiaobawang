...
from ..db.models.killmail import KillmailHighValueSubscription, KillmailConditionSubscription
...
class KillmailSubscriptionManager:
    ...
    async def get_high_value_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        try:
            query = select(KillmailHighValueSubscription).where(
                KillmailHighValueSubscription.is_enabled == True
            )
    ...
    async def get_condition_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        try:
            query = select(KillmailConditionSubscription).where(
                KillmailConditionSubscription.is_enabled == True
            )
            result = await self.session.execute(query)
            subscriptions = result.scalars().all()
    ...
    async def add_subscription(
        ...
        if sub_type == "high_value":
            return await self._add_high_value_subscription(
                platform, bot_id, session_id, session_type, min_value
            )
            ...
            return await self._add_condition_subscription(
                platform, bot_id, session_id, session_type,
                target_type, target_id, target_name,
                is_victim, is_final_blow, min_value
            )
        else:
    ...
    async def get_session_subscriptions(self, platform: str, bot_id: str, session_id: str, session_type: str) -> Dict[str, Any]:
        """获取特定会话的所有订阅信息"""
        try:
            # 查询高价值订阅
            ...
            result = {
                "high_value_subscription": None,
                "condition_subscriptions": []
            }

            if hv_subscription:
                result["high_value_subscription"] = {
                    "id": hv_subscription.id,
                    "min_value": hv_subscription.min_value
                }
